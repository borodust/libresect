...
resect_type_method resect_method_create(resect_visit_context visit_context, resect_translation_context context,
    ...
    method->decl = resect_decl_create(visit_context, context, cursor).decl;
...
void resect_method_free(resect_type_method method, resect_set deallocated) {
    ...
    if (method->decl != NULL) {
        resect_decl_free(method->decl, deallocated);
    }
    free(method);
}
...
resect_decl resect_type_method_get_decl(resect_type_method method) {
    return method->decl;
}
